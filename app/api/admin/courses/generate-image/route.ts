...
function getImageStyleForTopic(title: string, category: string): ImageStyle {
  ...
  if (/python|파이썬/i.test(title)) {
    return {
      ...
      elements: 'floating Python snake logo, colorful code blocks, data structures, algorithms visualization, binary code streams',
  ...
  if (/javascript|자바스크립트|js/i.test(title)) {
    return {
      ...
      elements: 'JavaScript logo, browser windows, DOM tree visualization, animated code snippets, glowing brackets and semicolons',
  ...
  if (/react|리액트/i.test(title)) {
    return {
      ...
      elements: 'spinning React atom logo, component hierarchy, virtual DOM tree, hooks icons, state management flow',
  ...
  if (/ai|인공지능|gpt|chatgpt|claude/i.test(title)) {
    return {
      ...
      elements: 'glowing neural network, AI brain visualization, chat bubbles, robotic elements, digital synapses, holographic interface',
  ...
  if (/바이브코딩|vibe|코딩/i.test(title)) {
    return {
      ...
      elements: 'colorful generative art patterns, code morphing into art, rainbow gradient code, creative geometric shapes, artistic algorithms',
  ...
  // 비즈니스/마케팅
  if (/스마트스토어|쇼핑몰|온라인.*셀링|셀러/i.test(title)) {
    return {
      scene: 'E-commerce success and online business',
      elements: 'floating product boxes, shopping cart icons, sales growth charts, smartphone with store app, money and coins, delivery icons',
  ...
  if (/마케팅|광고/i.test(title)) {
    return {
      ...
      elements: 'social media icons, megaphone, growth arrows, target audience, analytics dashboard, engagement metrics',
      colors: 'vibrant orange (#F97316) and energetic red (#EF4444)',
      mood: 'dynamic, strategic, impactful'
    }
  }
  if (/유튜브|영상|video|비됴/i.test(title)) {
    return {
      scene: 'Video content creation and YouTube success',
      elements: 'play button, camera lens, video timeline, subscriber count, thumbnail grid, studio lighting, microphone',
      colors: 'YouTube red (#FF0000) and clean white',
      mood: 'creative, engaging, professional'
    }
  }

  // 디자인
  if (/포토샵|photoshop|사진.*편집/i.test(title)) {
    return {
      scene: 'Photo editing and digital design',
      elements: 'layer panels, color adjustment tools, before/after images, brushes, magic wand, creative filters',
      colors: 'creative pink (#EC4899) and rich purple (#8B5CF6)',
      mood: 'creative, artistic, transformative'
    }
  }
  if (/피그마|figma|ui|ux/i.test(title)) {
    return {
      scene: 'UI/UX design and prototyping',
      elements: 'wireframe sketches, design components, user flow diagrams, mobile and web mockups, design system elements',
      colors: 'Figma purple and orange gradient',
      mood: 'clean, user-centered, systematic'
    }
  }
  ...
  if (/프로그래밍|개발/i.test(category)) {
    return {
      ...
      elements: 'code editor screens, terminal windows, git branches, deployment pipelines, cloud services',
  ...
  if (/비즈니스/i.test(category)) {
    return {
      ...
      elements: 'business charts, growth metrics, professional workspace, success indicators, strategy elements',
  ...
  if (/디자인/i.test(category)) {
    return {
      ...
      elements: 'color palettes, design tools, creative layouts, artistic elements, inspiration mood board',
  ...
  return {
    ...
    elements: 'floating books, digital screens, lightbulb ideas, knowledge icons, learning pathways',
